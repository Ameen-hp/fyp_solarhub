...
const HostOrderPage = () => {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = orders.filter((order) =>
      ...
      order.customerAddress.toLowerCase().includes(value)
    );
  ...
  const handleConfirm = async (orderId) => {
    try {
      await axios.post(`http://localhost:5000/api/order/confirmOrder/${orderId}`);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <h1 className="text-4xl font-bold text-center text-blue-800 mb-10">📦 Solar Components Orders</h1>
      ...
      {filteredOrders.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredOrders.map((order) => (
            ...
            >
              <h2 className="text-lg font-semibold text-gray-700 mb-2">{order.customerName}</h2>
